...
API_KEY = ""  # Chave da API para consulta
BASE_URL = "https://api.openweathermap.org/data/2.5/onecall"  # URL base para request das informações climáticas do dia
GEOCODING_URL = "http://api.openweathermap.org/geo/1.0/direct"  # URL base para request da localização geográfica (latitude e longitude) da cidade onde será criado o alerta

# O nome da cidade pode ser enviado só ou no formato CIDADE,CODIGO DO ESTADO, CODIGO DO PAÍS (Ex.: Piracicaba | Piracicaba,SP,BR | Piracicaba,SP)
city = input("Digite a cidade: ")

request_geo = f"{GEOCODING_URL}?q={city}&limit=1&appid={API_KEY}"  # Complemento para o request da localização geográfica
...
if send:
    ...
    password = ""  # Senha do email que enviará o alerta
